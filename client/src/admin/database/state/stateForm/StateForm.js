...
function StateForm(props) {
    ...
    useEffect(() => {
        (
            async () => {
                if (id) {
                    const response = await fetch(`${api}/state/getById`, {
                        method: 'POST',
                        headers: {
                    ...
                    if (content.data) {
                        const { data } = content;
                        setName({ value: data.name, error: false, helperText: 'Enter a name Ex. Alaska' });
    ...
    const handleNameChange = (event) => {
        if (event.target.value.length === 0) {
            ...
            setName({ value: event.target.value, error: false, helperText: 'Enter a name Ex. Alaska' });
        }
    }
    ...
    const handleSubmitAdd = async event => {
        ...
        const response = await fetch(`${api}/state/add`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
